        ...
        <Formik
          ...
          validate={(values)=> {
            ...
            if(selectedProduct.product) {
              if((quantity >= 1) && (quantity <= selectedProduct.product.avaliableQuantity)) {
                setSelectedProduct({...selectedProduct, quantity});                
  
              }else {
                errors.quantity = "The quantity must be minimum 1 and max " + selectedProduct.product.avaliableQuantity;
              }
            }else {
        ...
        >
          {({errors}) => (
            ...
            style={{width: "100%", display: "flex", flexDirection: "column", gap: "12px"}}>
              <TextField 
                title={`Cantidad (1 - ${selectedProduct.product ? selectedProduct.product.avaliableQuantity : selectedProduct.avaliableQuantity})`} 
                type={"number"} 
                name={"quantity"} 
              />
              <TextField 
